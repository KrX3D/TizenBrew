...
module.exports.onStart = function () {
  ...
  app.all('*', function (req, res) {
    if (req.url.startsWith('/module/')) {
      ...
      var upstreamUrl;
      var filePath = req.url.replace("/module/".concat(encodedModuleName, "/"), '');
      if (moduleName.includes('@')) {
        var _moduleName$split = moduleName.split('@'),
          _moduleName$split2 = _slicedToArray(_moduleName$split, 2),
          rawRepo = _moduleName$split2[0],
          tag = _moduleName$split2[1];
        var repo = getGitHubRepo(rawRepo);
        if (repo) {
          upstreamUrl = "https://raw.githubusercontent.com/".concat(repo, "/").concat(tag, "/").concat(filePath);
        } else {
          // npm package, fallback to jsDelivr
          upstreamUrl = "https://cdn.jsdelivr.net/".concat(moduleName, "/").concat(filePath);
        }
      } else {
        var _repo = getGitHubRepo(moduleName);
        if (_repo) {
          upstreamUrl = "https://raw.githubusercontent.com/".concat(_repo, "/main/").concat(filePath);
        } else {
          upstreamUrl = "https://cdn.jsdelivr.net/".concat(moduleName, "/").concat(filePath);
        }
      }
      fetch("".concat(upstreamUrl).concat(cacheBuster)).then(function (fetchRes) {
